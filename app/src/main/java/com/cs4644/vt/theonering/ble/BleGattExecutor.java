...
package com.cs4644.vt.theonering.ble;
...
public class BleGattExecutor extends BluetoothGattCallback {
    ...
    private ServiceAction serviceReadAction(final BluetoothGattService gattService, final String characteristicUuidString, final String descriptorUuidString) {
    ...
    private ServiceAction serviceNotifyAction(final BluetoothGattService gattService, final String characteristicUuidString, final boolean enable) {
        return new ServiceAction() {
            @Override
            public boolean execute(BluetoothGatt bluetoothGatt) {
    ...
    private ServiceAction serviceIndicateAction(final BluetoothGattService gattService, final String characteristicUuidString, final boolean enable) {
        return new ServiceAction() {
            @Override
            public boolean execute(BluetoothGatt bluetoothGatt) {
                if (characteristicUuidString != null) {
    ...
    private ServiceAction serviceWriteAction(final BluetoothGattService gattService, final String uuid, final byte[] value) {
        return new ServiceAction() {
            @Override
            public boolean execute(BluetoothGatt bluetoothGatt) {
    ...
    protected void execute(BluetoothGatt gatt) {
        if (mCurrentAction == null) {
            while (!mQueue.isEmpty()) {
                final ServiceAction action = mQueue.pop();
